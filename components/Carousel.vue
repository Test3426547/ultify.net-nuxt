<template>
  <section class="bg-ultify-grey h-screen relative overflow-hidden flex flex-col justify-center items-center">
    <div class="container mx-auto px-4 flex flex-col justify-center items-center h-full w-full">
      ...
      <div v-else-if="carouselData" class="flex flex-col items-center w-full h-full">
        ...
        <div class="relative w-full flex-grow flex items-center">
              ...
              <div v-for="(card, index) in groupedCards" :key="index" class="w-full flex-shrink-0 flex space-x-8">
                <div v-for="(item, itemIndex) in card" :key="itemIndex" class="w-1/2">
                  <a :href="item.link" class="block relative group overflow-hidden rounded-lg shadow-lg hover:scale-105 transition-transform duration-300 ease-in-out">
                    <img :src="item.image.url" :alt="item.image.alternativeText || item.image.name" class="w-full h-[50vh] object-cover" />
          ...
          <button @click="prevSlide" class="absolute top-1/2 -translate-y-1/2 -left-8 w-16 h-16 bg-ultify-blue bg-opacity-20 rounded-full flex items-center justify-center group hover:bg-opacity-30 transition-all duration-300 focus:outline-none">
            <svg class="w-8 h-8 text-ultify-blue group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
          </button>
          <button @click="nextSlide" class="absolute top-1/2 -translate-y-1/2 -right-8 w-16 h-16 bg-ultify-blue bg-opacity-20 rounded-full flex items-center justify-center group hover:bg-opacity-30 transition-all duration-300 focus:outline-none">
            <svg class="w-8 h-8 text-ultify-blue group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
